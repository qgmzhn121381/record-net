...
export default function AuthPage() {
  ...
  const [birthdayType, setBirthdayType] = useState('solar');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isLogin && password !== confirmPassword) { setError('两次密码不一致'); return; }
    ...
    try {
      ...
      if (!isLogin && birthday) {
        ...
        body.birthdayType = birthdayType;
      }
      ...
      <div className="login-container">
        ...
        <div className="login-card">
          ...
          <form onSubmit={handleSubmit} className="login-form">
                ...
                <div className="login-field">
                  ...
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <input type="date" value={birthday}
                      ...
                      className="login-input" style={{ flex: 1 }} />
                    <select value={birthdayType}
                      onChange={(e) => setBirthdayType(e.target.value)}
                      className="login-input" style={{ width: '100px' }}>
            ...
            {error && (
              <p className={`login-error ${error.includes('成功') ? 'success' : ''}`}>{error}</p>
            )}
